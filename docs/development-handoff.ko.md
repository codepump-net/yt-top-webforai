...
현재 45개 경로를 구현했습니다. 병원 소개, 의료진 2명, 심장·초음파·내시경·내과 진료, 검진 안내, 건강정보 4편, 사례 원문 목록, 공지 채널 안내, 방문·비용·개인정보·검색·전체 페이지·404를 포함합니다. 사례 25편의 독립 요약, 공지 5편의 복제본, 작성 원칙 페이지는 공개 범위에서 제외했습니다. Next.js가 별도로 만드는 내부 오류 페이지는 이 수에 포함하지 않습니다.
...
| 대상 | 파일 | 변경 시 확인 |
|---|---|---|
| 45개 페이지 원고·질문·출처·관련 링크 | `content/pages.json` | 제목·요약·본문·출처를 함께 수정, 검수 갱신 |
| 페이지별 환자의 필요 | `content/page-intents.json` | 새 페이지의 환자용 목적 기록; 내부 검사에만 사용 |
| 기존 병원 사례 25개 연결 | `content/case-links.json` | 원문 링크·짧은 설명·출처 일치 확인 |
...
| 출처·신뢰의 검토 가능성 | 주제를 구분한 병원·기관 자료 링크, 실제 승인 후에만 검토자 표시; 제작·검수 상태는 내부 기록 |
...
워크플로는 루트 경로와 저장소 하위 경로에서 각각 설치 → 취약점 검사 → lint → 단위 검사 → 기존 Python 하네스 검사 → 45페이지 빌드/감사 → 타입 검사 → 브라우저/접근성/성능 검사 순으로 실행합니다. 둘 다 성공해야 Pages를 배포합니다. PR은 검토 모드로 검사하며 배포하지 않습니다. 빌드 시 환자용 페이지 목적과 내부 문구·내부 파일 유출도 검사합니다.

main 배포 직전 원격 main의 SHA를 확인하므로 오래된 실행이 최신 사이트를 덮어쓰지 못합니다. `cancel-in-progress: false`로 배포 중단을 피합니다. Pages 배포 후 같은 실행의 `checks-project` artifact에서 기대 매니페스트를 가져와 45페이지 실제 HTML 해시, canonical, noindex, 없는 경로의 HTTP 404를 검사합니다. `build-manifest.json`은 `reports/`에만 생성하며 공개 사이트에서 다운로드하지 않습니다. 로컬에서 `verify:live`를 실행할 때도 해당 배포의 `reports/build-manifest.json`이 필요합니다.
...
4. 실제 검토 결과만 `content/reviews.json`에 입력합니다. 각 indexable 페이지에 `pageId`, `status: approved`, `reviewer`, `role: medical 또는 operations`, ISO 8601 `reviewedAt`·`expiresAt`, `evidence`, `digest`가 필요합니다. 의료 설명 페이지는 medical 역할과 실제 프로필의 `reviewerId`가 필요하며 이름이 일치해야 합니다. 개인정보가 있는 원본 검토 기록은 비공개로 보관하고 evidence에는 내부 기록 식별자나 공개 가능한 확인 설명만 씁니다.
...
6. 기존 사이트와 병행할 주소와 검색 공개 범위를 확인한 후 `PUBLICATION_MODE=production`으로 변경하고 Actions를 실행합니다. 실제 배포의 canonical·sitemap·noindex 여부를 확인한 다음 Google Search Console·네이버 서치어드바이저의 소유권 인증·사이트맵 제출을 진행합니다. 계정이나 인증 코드를 추정해서 넣지 않습니다.

운영 호스팅은 [GitHub Pages 이용 제한](https://docs.github.com/en/pages/getting-started-with-github-pages/github-pages-limits)을 포함해 확인합니다. 제품의 목적은 병원 소개와 진료·방문 안내입니다. 결제·환자 정보 입력·온라인 예약 처리 기능은 없으며, 전화와 지도 링크를 제공합니다.

## AI 인용 관측 하네스

`content/measurement-queries.json`에는 편집 질문 32개가 있습니다. 기존 20개에 조사 기반 12개를 추가했으며 Q03은 기존 두근거림 질문에 연결했습니다. 검증된 검색 빈도나 독립 환자 수요 순위가 아니며 `frequencyEvidence`는 null입니다. 지식인 이용자의 원문이나 개인정보를 포함하지 않습니다.
...
`originalSites`에 정의한 원 사이트 인용과 신규 사이트 인용, 둘의 합집합을 각각 집계합니다. 같은 답변에 둘 다 있어도 합집합에서는 한 번 셉니다. 관측에는 가능한 `model`, `region`, `searchMode`, `querySetVersion`도 기록하세요. 값이 다른 조건은 같은 집계로 섞지 않으며 누락된 조건은 null로 남깁니다. 새로운 질문 집합을 과거 관측의 버전으로 임의 입력하지 않습니다.
